...
func TestProjectHealth_KongParse_Default(t *testing.T) {
	...
	_, err = p.Parse([]string{"project", "health", "PROJ"})
	if err != nil {
		t.Fatalf("パースエラー: %v", err)
	}

	cmd := root.Project.Health
...
func TestProjectHealth_KongParse_WithFlags(t *testing.T) {
	...
	_, err = p.Parse([]string{"project", "health", "PROJ", "--days", "14", "--include-comments", "--exclude-status", "完了,対応済み"})
	if err != nil {
		t.Fatalf("パースエラー: %v", err)
	}
...
func TestProjectHealth_KongParse_MissingProjectKey(t *testing.T) {
	...
	_, err = p.Parse([]string{"project", "health"})
	if err == nil {
