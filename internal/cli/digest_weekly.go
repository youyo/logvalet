...
// DigestWeeklyCmd は lv digest weekly コマンド。
// 指定プロジェクトの週次ペリオディックダイジェストを生成する。
type DigestWeeklyCmd struct {
	// ProjectKey はプロジェクトキー（必須）。
	ProjectKey string `short:"k" required:"" help:"project key"`
	// Since は集計開始日（YYYY-MM-DD, 省略時は 7 日前）。
	Since string `help:"since date (YYYY-MM-DD, default: 7 days ago)"`
	// Until は集計終了日（YYYY-MM-DD, 省略時は now）。
	Until string `help:"until date (YYYY-MM-DD, default: now)"`
}

// Run は digest weekly コマンドを実行する。
